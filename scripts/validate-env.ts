...
const ENV_CONFIG: Record<string, EnvVarConfig> = {
  ...
  // Токена у погоды нет намеренно: Open-Meteo отдаёт ветер и температуру по
  // координатам без ключа и регистрации. Ключ означал бы ещё один секрет в
  // окружении и ещё одну причину, по которой погода однажды перестанет
  // работать на проде.
