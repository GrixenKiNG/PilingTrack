...
-- ------------------------------------------------------------
-- Зачем. Приложение, воркеры и ws ходили в базу ролью-владельцем (`piling` на
-- проде, `postgres` локально), а это суперпользователь (проверено 13.08.2026:
-- rolsuper=t, rolbypassrls=t). Суперпользователь обходит RLS ВСЕГДА: FORCE ROW
-- LEVEL SECURITY на него не распространяется — то есть все политики
-- tenant_isolation_* были декоративны, а защищал только прикладной фильтр
-- (tenantWhere). См. ADR-0044 и ADR-0042 п. 7.
--
-- Состояние на 22.09.2026: прод переключён (13.08.2026, ранбук 011), локальная
-- разработка — тоже (22.09.2026). Роль-владелец осталась за миграциями, сидами
-- и одноразовыми базами интеграционных тестов.
--
