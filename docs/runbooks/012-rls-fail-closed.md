...
## Репетиция на копии прода — проведена 19.08.2026

Снимок прода (`e948467`, 18.07.2026, 160 отчётов, 17 МБ) развёрнут локально,
накачены все 31 миграция, приложение поднято под `pilingtrack_app` без
`BYPASSRLS`. Прод не изменялся: только `pg_dump` и чтение.

| Что мерили | Результат |
|---|---|
| Накат 31 миграции на боевых данных | **19 секунд**, ни одного отказа |
| Политики после наката | 46 строгих, ноль в режиме аудита |
| Обход 63 маршрутов чтения | 48 с данными, 0 отказов доступа |
| Вход по паролю и по ПИН-коду | 200 (ПИН-код — через роль опознания) |
| Аналитика (сырой SQL) | 28 094 м, 8 установок, объекты с цифрами |
| Пересчёт проекций | 160 / 90 / 23 строки |
| Запись отчёта → outbox → проекция | Отчётов 161, аналитики 161 |
| Изоляция под ролью без BYPASSRLS | Без организации 0, под `orion` 161, под чужой 0 |

**Три вещи, которых в этом ранбуке не было — теперь учтены ниже:** дозаполнение
техготовности, вид центра готовности в первый день и всплеск отставания outbox
...
### Дозаполнение техготовности — обязательный шаг

Центр готовности после наката пуст: снимков ещё нет ни у одной установки.
Заполняется одним прогоном (на копии прода — 3 секунды, 8 установок):

```bash
docker compose exec -T -e DATABASE_URL="$DATABASE_URL_POSTGRES" app   npx tsx scripts/backfill-tech-readiness.ts
```

`DATABASE_URL_POSTGRES` здесь не случайность: скрипт ходит в базу напрямую, и
под ролью приложения строгие политики не отдадут ему ни строки (проверено).

**Чего ждать в первый день.** После дозаполнения все установки покажут
«не допущена», балл 0 — это правильно: осмотров по новым правилам ещё никто не
проводил. Балл появится, когда механики пройдут первый чек-лист. Людей стоит
предупредить заранее, иначе это выглядит как поломка.

### Всплеск отставания outbox сразу после переключения

В первые минуты после подъёма воркеров придёт предупреждение
`Outbox lag is NNs (warn threshold: 60s)` — воркеры разбирают накопившееся и
переизбирают лидера. На репетиции очередь разошлась сама, проекция догнала
отчёт. Тревожиться стоит, только если отставание растёт больше 10 минут:

```bash
docker compose exec -T postgres psql -U piling -d pilingtrack -c   'SELECT count(*) FROM "OutboxEvent" WHERE published=false OR projected=false;'
```

Исторические 865 строк outbox получат пустую организацию (колонка появляется
этими же миграциями), но переразбору не подлежат: обе колонки-потребителя у них
уже проставлены. Проверено на снимке — ноль неразобранных.
