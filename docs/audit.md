...
### A-13. Бэкапы и медиа под одними ключами — ЗАКРЫТО для новых копий (16.09.2026)
...
**Хвост, оставшийся открытым:** в общем бакете `pilingtrack/db-backups/`
лежат **79 исторических дампов (19.7 МБ)**, включая свежие. Их по-прежнему
достаёт токен приложения, то есть утечка ключа из контейнера раскрывает
79 копий базы целиком. Разделение действует только для копий, созданных
после 16.09.2026. Перенести и удалить старые — отдельное решение владельца
(удалять только после проверки, что перенос состоялся).
