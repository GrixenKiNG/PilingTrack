...
### A-13. Бэкапы и медиа под одними ключами — частично закрыто (15.09.2026)
...
**Что остаётся за владельцем** (код это сделать не может):

1. В Cloudflare R2 создать **отдельный бакет** для дампов (например
   `pilingtrack-backups`).
2. Выпустить **отдельный API-токен**, ограниченный этим бакетом (Object
   Read & Write). Токен приложения к нему доступа иметь не должен.
3. Прописать четыре `BACKUP_S3_*` в `/opt/pilingtrack/.env`.
4. Дождаться ночного прогона и убедиться, что в логе
   `Off-site credentials: BACKUP_S3_* (dedicated backup credentials)`.
5. Включить на бакете versioning / object-lock — тогда удаление обратимо.
