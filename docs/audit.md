...
**Проверка 22.09.2026 (по живому проду, не по докам).** Внешнее ревью
сообщало о кросс-тенантном IDOR в `/api/auth/me`, «эксплуатируемом в текущем
деплое». Проверено:

| Что | Факт |
|-----|------|
| Роль приложения на проде | `pilingtrack_app`, `rolsuper=f`, `rolbypassrls=f` |
| `"User"` | `relrowsecurity=t`, `relforcerowsecurity=t` |
| Политика | `tenantId = current_setting('app.current_tenant', true)` — fail-closed |

Вывод: **на бою кросс-тенантного чтения нет**, ревью тестировало локальную
конфигурацию (там `DATABASE_URL` шёл суперпользователем) и перенесло вывод на
прод. Подтвердилось другое, и оно закрыто в тот же день:

1. `/api/auth/me` выбирал строку по одному первичному ключу и держался на
   единственном слое (RLS). Добавлен `ensureTenantAccess` — второй слой, чужой
   тенант отдаёт такой же 404, как отсутствующая строка.
2. `tests/integration/rls-tenant-enforcement.spec.ts` пропускался **молча**:
   `DATABASE_URL_APP_ROLE` не задавалась нигде, то есть изоляцию тенантов
   не проверял ни один зелёный прогон. Теперь адреса базы подставляются из
   `.env` (`vitest.config.ts`), CI заводит роль и объявляет пропуск падением,
   а сам набор сеет свою строку и не зависит от данных в базе.
3. Локальная разработка переведена на `pilingtrack_app` — модель прав та же,
   что на бою. Владелец остался за миграциями, сидами и одноразовыми базами
...
### A-14. Остаточные уязвимости npm — принятый риск (15.09.2026)

Прогон `npm audit` 15.09.2026: было 13 (1 critical, 8 high, 4 moderate), стало
**7 (0 critical, 4 high, 3 moderate)**. Закрыты безопасным `npm audit fix`
(только вперёд, без даунгрейдов): `fast-uri` 3.1.5→3.1.7 (4 advisory про
SSRF/host-confusion), `js-yaml`, `browserslist`, `baseline-browser-mapping`,
`sharp` 0.35.3→0.35.4. Critical закрыт вручную: `next` 16.2.12 → **16.3.5**
(GHSA-p293-qw3h-jr36 — неаутентифицированный RCE на Windows-хостах;
GHSA-2xp9-vwfh-vxw4 — RCE в Image Optimization API при AVIF).

**Что осталось и почему не чинится:**

| Пакет | Severity | Путь | Почему принят |
|---|---|---|---|
| `deepmerge-ts` <8 | high | `prisma → @prisma/config` | Транзитивный. Единственный доступный «фикс» — `npm audit fix --force`, который **откатывает Prisma 7.10.0 → 6.19.3** (мажор назад, несовместимо с 101 применённой миграцией). Лечение хуже болезни. |
| `mysql2` <=3.23.0 | high | `prisma` | Тот же путь. MySQL в проекте **не используется** — драйвер приезжает как опциональная зависимость Prisma и в рантайме не грузится. |
| `@vitest/mocker`, `vitest`, `@vitest/coverage-v8` | moderate | devDependencies | Дев-тулинг, в прод-образ не попадает. |

**Условие пересмотра:** релиз Prisma с обновлённым `@prisma/config`. До тех пор
пункт закрыт как принятый риск — **не поднимать заново как P0**.

⚠️ **Не запускать `npm audit fix --force`** в этом репозитории: он утащит Prisma
на мажор назад. Безопасный путь — `npm audit fix --dry-run`, глазами убедиться,
что в списке нет отката `prisma`, и только потом применять.

### A-13. Бэкапы и медиа под одними ключами — ЗАКРЫТО (16.09.2026)

**Находка.** `scripts/backup-postgres.sh` переиспользовал те же ключи S3, что
приложение использует для фотографий: один бакет `pilingtrack`, дампы в
`db-backups/`, медиа в `media/`, один токен на всё. Копия off-site
(Cloudflare R2, вне периметра VPS), но **не off-credential**: утечка, ротация
или отзыв ключа приложения уносит вместе с собой все копии базы. Отказы
скоррелированы — а это ровно то, от чего off-site копия должна защищать.

Ни один из двух аудитов этого не увидел: находка не грепается, к ней приходишь
только сопоставив скрипт бэкапа с `.env`.

**Что сделано в коде.** Скрипт теперь предпочитает выделенные
`BACKUP_S3_ENDPOINT / BACKUP_S3_BUCKET / BACKUP_S3_ACCESS_KEY_ID /
BACKUP_S3_SECRET_ACCESS_KEY`, а при их отсутствии откатывается на `S3_*` —
поведение существующих установок не меняется. В лог пишется, какой набор
использован. Переменные описаны в `.env.production.example`.

**Выполнено и подтверждено исполнением 16.09.2026.** Владелец создал бакет
`pilingtrack-backups` и отдельный токен, ограниченный только им; четыре
`BACKUP_S3_*` прописаны в `/opt/pilingtrack/.env`. Прод на `0e164ba`.

Прогон штатной задачи: код возврата 0,
`Off-site credentials: BACKUP_S3_* (dedicated backup credentials)`,
копия ушла в `R2:pilingtrack-backups/db-backups/`.

Изоляция проверена в обе стороны, не по настройкам, а запросами:

| Кто | Куда | Результат |
|---|---|---|
| резервный токен | `pilingtrack-backups` | видит |
| резервный токен | `pilingtrack` (медиа) | **403 AccessDenied** |
| токен приложения | `pilingtrack-backups` | **403 AccessDenied** |
| токен приложения | `pilingtrack` | видит |

**Исторические копии перенесены и удалены из общего бакета.** 79 дампов
скопированы в `pilingtrack-backups`, сверка `rclone check --one-way --checksum`
дала «0 differences found, 79 matching files», после чего исходники удалены из
`pilingtrack/db-backups/`.

Состояние после переноса:

| | До | После |
|---|---|---|
| `pilingtrack/media/` | 73 | **73** (не затронуты) |
| `pilingtrack/db-backups/` | 79 | **0** |
| `pilingtrack-backups/db-backups/` | 2 | **81** |

Общий бакет теперь содержит ровно 73 объекта — только медиа. Контрольный прогон
штатной задачи после удаления: код возврата 0, копия ушла в выделенный бакет.
Утечка ключа приложения больше не раскрывает ни одной копии базы.

Дополнительно на VPS хранится 35 свежих дампов (`/var/backups/pilingtrack/`,
очистка по `-mtime +RETENTION_DAYS`), то есть у недавних копий есть второй
экземпляр.

**Остаётся необязательным:** `Bucket Lock Rules` на `pilingtrack-backups`
(в R2 нет отдельного версионирования — блокировка его заменяет и строже:
объект нельзя удалить до конца срока никому). Рекомендуемый срок — 30 дней;
учтите, что блокировка конфликтует с правилами жизненного цикла.

**Отдельно не закрыто:** у медиа (`media/`) вообще нет процедуры
восстановления — это единственная копия, а не бэкап. Versioning на бакете
приложения — минимальная мера.
