...
| 13. Послесменный осмотр по разделам «После смены» | готово | `7a6c467` |
...
### Задача 13: послесменный осмотр (20.08.2026)

Отдельный шаблон не понадобился. Разделы конца смены уже лежат в действующих
блоках: «После смены» (LRH 100, Woltman-PVE 50PR, КБУРГ-16), «После работы»
(Jintai SD-20), «Контроль в процессе работы» и «После окончания смены»
(гидромолот PVE 7NL). Собранный чек-лист делится надвое по заголовку раздела —
`src/modules/inspections/domain/phase-split.ts`.

Проверено на живой базе для машины оператора (PVE 50PR + гидромолот):
предсменный осмотр 108 пунктов, послесменный 15, осмотр вне смены не делится
и остаётся целым (123). Повторный запуск той же фазы возвращает начатый осмотр,
а не заводит второй.

Попутно закрыт дефект, который делал прежнюю работу бесполезной: осмотр,
заведённый с общей формы `/inspections/new`, не получал `shiftId`, поэтому экран
оператора его не находил и бесконечно предлагал «начать осмотр», а сравнение
состояний вход/выход оставалось без данных. Теперь осмотр обеих фаз заводится
прямо с экрана смены — с установкой из экипажа и проставленной сменой.

Миграция `20260820140000_inspection_shift_phase_unique`: уникальный индекс
`(tenantId, shiftId, phase)` вместо обычного. NULL в Postgres различны, поэтому
осмотры вне смены ограничением не задеты.
