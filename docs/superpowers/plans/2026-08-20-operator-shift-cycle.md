...
| 14. Оператор открывает смену и выбирает установку | готово | `ac6d4bb` |
| 15. Пошаговый обход осмотра по узлам | готово | `b93d1a8` |
...
### Задачи 14–15 (20.08.2026)

**Оператор открывает смену сам.** Администратор закрепляет за ним список
установок (снят уникальный индекс `Crew.operatorId`), оператор выбирает машину
и открывает смену. Установка по-прежнему принадлежит одной активной бригаде —
двум операторам одну машину пока не разделить, это отдельный вопрос. Граница
«только своя установка» — в команде (`requireOperatorAssignment`), а не в
списке прав. Списанные установки (`isActive = false`) в списке не предлагаются.

**Пошаговый обход.** Осмотр идёт по разделам шаблона: раздел = узел. Отдельной
таблицы соответствия «раздел → узел» нет намеренно — она разошлась бы с
шаблонами, как только механик их поправит. Завершение доступно на последнем
узле, переход между узлами молча сохраняет черновик.

Проверено в браузере: оба пути пройдены целиком — выбор установки, открытие
смены, создание предсменного осмотра со смены, обход 17 разделов, сохранение
фото при смене раздела, экран 375 px без горизонтальной прокрутки. Записи,
заведённые проверкой, удалены.

Осталось непроверенным глазами: осмотр после работ (нужна идущая смена с
закрытым предсменным осмотром) и режим чтения подписанного осмотра.

Ранее не сделанное намеренно (закрыто задачей 15): пошаговый чек-лист ЕО «по узлам». Существующий экран
