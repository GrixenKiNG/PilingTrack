...
- [ ] Написать падающие тесты для обязательного ответа, фотографии при дефекте, критической блокировки и «не применяется».
- [ ] Выполнить обязательный GitNexus `impact` для изменяемых символов контрактов.
- [ ] Реализовать чистую доменную оценку без обращений к базе.
- [ ] Подтвердить, что критический ответ запрещает продолжение, а важный формирует предупреждение или блокировку по правилу.
- [ ] Запустить оба набора доменных тестов.
...
- [ ] Написать интеграционный тест версионированного снимка шаблона и неизменяемых ответов.
- [ ] Запустить тест и зафиксировать ожидаемое падение из-за отсутствующих таблиц.
- [ ] Выполнить анализ влияния изменяемых моделей Prisma.
- [ ] Добавить tenant-scoped таблицы, внешние ключи, уникальность клиентской команды и индексы чтения смены.
- [ ] Хранить снимок текста и правил шаблона внутри выполнения, чтобы последующее редактирование шаблона не меняло историю.
- [ ] Сгенерировать клиент Prisma и запустить интеграционный тест.
...
- [ ] Сначала расширить контрактные тесты командами, допустимыми и недопустимыми полезными данными.
- [ ] Запустить контрактный тест и подтвердить отказ неизвестных команд.
- [ ] Выполнить GitNexus impact для реестра и `executeOperatorCommand`; предупредить при высоком риске.
- [ ] Зарегистрировать команды, повторно используя общий конверт, версии, контрольную сумму и идемпотентность.
- [ ] Для критических решений сохранить запрет автономного серверного подтверждения; разрешить автономный сбор доказательств по действующей политике.
- [ ] Запустить контрактные и командные модульные тесты.
...
- [ ] Написать падающие тесты переходов по всем восьми этапам и каждой обязательной блокировке.
- [ ] Выполнить impact для `resolveOperatorWorkplace`; проверить прямых потребителей API.
- [ ] Реализовать проекцию без удаления текущих полей безопасности, ремонта, передачи и синхронизации.
- [ ] Заполнить `workZone`, доказательства готовности, контакты и подробности профиля фактическими данными или явным состоянием «данные не предоставлены».
- [ ] Запустить доменные, контрактные и интеграционные тесты рабочего места.
...
- [ ] Написать падающие тесты строгого снимка восьми этапов и новых типов ответа.
- [ ] Написать падающий тест сохранения ответа с фотографией и повторной идемпотентной отправки.
- [ ] Выполнить impact для изменяемых схем и реестра действий.
- [ ] Расширить Zod-схемы и очередь без ослабления `.strict()`.
- [ ] Убедиться, что автономный черновик не превращается в авторитетное решение о допуске.
- [ ] Запустить тесты контрактов и автономного контура.
...
- [ ] Написать тесты пяти вкладок, этапа «N из 8», закреплённого действия и безопасной области.
- [ ] Запустить тест и подтвердить падение на текущей семиколоночной ленте.
- [ ] Выполнить impact для `OperatorWorkplaceScreen` и `PhaseRoute`.
- [ ] Реализовать одноколоночный телефонный контейнер `max-w-[430px]`, фон и компоненты текущей дизайн-системы.
- [ ] Удалить горизонтальную семиколоночную ленту из мобильного представления; добавить раскрываемый маршрут.
- [ ] Сохранить полноэкранную безопасную остановку выше обычной навигации.
