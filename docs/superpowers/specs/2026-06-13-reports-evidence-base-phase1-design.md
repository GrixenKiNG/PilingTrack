...
- Маппер `statusLabel(status)`: draft→«Черновик», submitted→«Отправлен», pending→«На проверке», иначе значение; цвет бейджа по статусу.
...
- Приёмка (статус «Принят» + действие + права).
- Фильтры «Без PDF» (PDF нигде не хранится — нужен учёт генераций), «На проверке», «Исправленные» (`version>1`).
