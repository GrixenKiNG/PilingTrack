  ...
  if (!block) {
    return (
      <section aria-label="Свойства плитки" className="space-y-3">
        <h3 className="text-sm font-semibold text-foreground">Размер плитки</h3>
    ...
    <section aria-label="Свойства блока" className="space-y-3">
      ...
      {block.kind === 'image' && (
        <div className="space-y-3 rounded-xl border border-border bg-card p-3">
          {imageTools && <p className="text-xs font-semibold text-foreground">Фото: {imageTools.subjectName}</p>}
          <label className="space-y-1 text-xs font-medium text-muted-foreground">
          ...
          {imageTools && (
            <label className="block min-h-11 cursor-pointer rounded-lg border border-border px-3 py-3 text-xs font-semibold text-foreground hover:bg-muted">
              Заменить фото
              <input
      ...
      <div className="grid grid-cols-2 gap-2">
        <label className="space-y-1 text-xs font-medium text-muted-foreground"><span>Цвет текста</span><input aria-label="Цвет текста" className={`${inputClass} p-1`} type="color" value={block.style.color} onChange={(event) => changeStyle({ color: event.target.value })} /></label>
        <label className="space-y-1 text-xs font-medium text-muted-foreground"><span>Цвет фона</span><input aria-label="Цвет фона" className={`${inputClass} p-1`} type="color" value={block.style.background} onChange={(event) => changeStyle({ background: event.target.value })} /></label>
        <label className="space-y-1 text-xs font-medium text-muted-foreground"><span>Цвет границы</span><input aria-label="Цвет границы" className={`${inputClass} p-1`} type="color" value={block.style.borderColor} onChange={(event) => changeStyle({ borderColor: event.target.value })} /></label>
      </div>
    </section>
  );
}
