 ...
 *
 * На телефоне плитка занимает всю строку: при 375px в половине строки плитке
 * остаётся ~165px, а ей нужно ≥250px (80 значок + 32 поля + текст), и значение
 * с подписью рассыпаются на переносы. Две колонки начинаются со sm, где ширины
 * уже хватает, а «один ряд» включается с lg. Чтобы плитке хватало ширины,
 * KPI-бар должен идти во всю ширину страницы — над областью с боковой панелью,
 * а не внутри левой колонки (иначе 6 плиток дают ~130px, и иконка с подписью не
 * помещаются).
 */
export const KPI_GRID = 'grid grid-cols-1 gap-3 sm:grid-cols-2 lg:[grid-template-columns:repeat(var(--kpi-cols),minmax(0,1fr))]';
