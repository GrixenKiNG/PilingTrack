...
const facts = (patch: Partial<OperatorShiftFacts> = {}): OperatorShiftFacts => ({
  ...
  equipment: { id: 'eq-1', name: 'Liebherr LRH 100 №1', model: 'LRH 100', engineHoursTotal: null, nextMaintenanceAtHours: null },
...
describe('resolveShiftPhase', () => {
  ...
  it('просроченное удостоверение не пускает к работе', () => {
    const phase = resolveShiftPhase(
      facts({ clearance: { blockers: ['Просрочен: Удостоверение машиниста — 12 дней'], warnings: [], documents: [] } }),
      'op-1',
    );
  ...
  it('начатую смену просроченный документ не останавливает', () => {
    const phase = resolveShiftPhase(
      facts({
        shift: { id: 'shift-1', state: 'STARTED', version: 2, type: 'DAY', productionDate: '2026-08-20', startedAt: null },
        clearance: { blockers: ['Просрочен: Удостоверение машиниста — 1 день'], warnings: [], documents: [] },
      }),
      'op-1',
    );
  ...
  it('свою передачу не предлагает принимать', () => {
    const mine = resolveShiftPhase(facts({
      incomingHandover: { id: 'h-1', shiftId: 'shift-0', summary: 'Течь шланга', submittedById: 'op-1', submittedByName: null, version: 1 },
    }), 'op-1');
    expect(mine.target).toBeNull();
    expect(mine.blockers[0]).toMatch(/другой оператор/i);

    const foreign = resolveShiftPhase(facts({
      incomingHandover: { id: 'h-1', shiftId: 'shift-0', summary: 'Течь шланга', submittedById: 'op-2', submittedByName: null, version: 1 },
    }), 'op-1');
