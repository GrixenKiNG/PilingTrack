    ...
    <div className="flex flex-col min-h-screen bg-muted">
      <div className="flex-1 overflow-y-auto px-4 pt-12 pb-28">
        <div className="flex flex-col items-center text-center">
          ...
          <h1 className="mt-4 text-2xl font-bold text-foreground">Отчёт отправлен!</h1>
          <p className="mt-1 text-sm text-muted-foreground">Передано диспетчеру · сохранено в {time}</p>
        </div>

        <div className="mt-8 mx-auto w-full max-w-sm rounded-xl bg-card border shadow-sm divide-y">
...
function Row({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between px-4 py-3">
      ...
      <span className="text-sm font-semibold font-mono tabular-nums text-foreground text-right">{value}</span>
    </div>
  );
}
