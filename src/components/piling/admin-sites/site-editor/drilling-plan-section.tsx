...
import { Drill, Plus, X } from '@/components/piling/icons/unified-icons';
...
export function DrillingPlanSection({ plans, setPlans }: DrillingPlanSectionProps) {
  return (
    <div className="space-y-2">
      ...
      {plans.length === 0 ? (
        <p className="text-xs text-muted-foreground py-1">Нет запланированного бурения</p>
      ) : (
          ...
          {plans.map((row, idx) => (
            <div key={row.tempId} className="flex items-center gap-1.5 bg-muted rounded-lg p-2">
              ...
              <button
                ...
                aria-label="Удалить строку плана бурения"
