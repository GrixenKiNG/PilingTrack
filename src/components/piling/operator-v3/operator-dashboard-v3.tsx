...
/**
 * Экран оператора из сборки v2.6.0 (`D:\PillingR\v2.6.0`), перенесённый
 * сюда как версия v3 — для сравнения с действующим `/operator` и с макетами
 * v5/v7/v10.
 *
 * Перенос ДОСЛОВНЫЙ. Это снимок того, как экран выглядел до контура
 * готовности: одна крупная кнопка «Создать отчёт», выбор объекта и три
 * последних отчёта. Ни шагов смены, ни осмотра, ни моточасов, ни передачи
 * смены здесь нет — они появились позже и живут в `operator-dashboard.tsx`.
 * Подтягивать их сюда нельзя: сравнивать имеет смысл только с оригиналом.
 *
 * Данные живые и те же, что у действующего экрана: `/api/sites`,
 * `/api/reports/my`. Собственной нижней панели у экрана нет — он рассчитывает
 * на общую из раскладки `(app)`, поэтому в `operatorRouteOwnsNavigation`
 * маршрут не добавлен.
 */
