  ...
  return (
    <div className="mx-auto max-w-xl space-y-5 p-4 pb-28 sm:p-5">
      <header className="flex items-center justify-between">
        <div>
          <p className="text-2xl font-bold text-slate-900">Оператор</p>
          ...
          {SHIFT_STEPS.map((step, index) => {
            ...
            return (
              <div key={step} className="relative flex flex-col items-center text-center">
                ...
                <span className="mt-2 text-sm leading-tight text-slate-600 sm:text-sm">{step}</span>
              </div>
            );
          })}
        </div>
      ...
      <div className="space-y-1.5">
        ...
        {sites.length > 0 ? (
          ...
          <div className="flex h-12 items-center justify-center rounded-md border border-dashed border-slate-300 bg-white text-sm text-slate-600">
      ...
      {reports.length > 0 && (
        <section className="rounded-xl border border-slate-200 bg-white p-3">
          <div className="mb-1 flex items-center justify-between">
            <h2 className="text-base font-semibold uppercase tracking-wider text-slate-600">Последние отчёты</h2>
            <button type="button" onClick={() => router.push('/history')} className="text-sm font-medium text-blue-700">История</button>
          </div>
            ...
            {reports.slice(0, 3).map((report) => (
              <li key={report.id}>
                <button type="button" onClick={() => router.push(`/history?reportId=${report.id}`)} className="flex w-full items-center justify-between py-3 text-left">
                  <span>
                    <span className="block text-base font-medium text-slate-900">{report.siteName}</span>
                    <span className="block text-sm text-slate-600">{new Date(report.date).toLocaleDateString('ru-RU')}</span>
                  </span>
