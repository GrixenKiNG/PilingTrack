    ...
    <div>
      ...
      {loading ? (
        ...
        <button
          ...
          onClick={() => inputRef.current?.click()}
          disabled={busy}
          className="flex h-28 w-full flex-col items-center justify-center gap-1.5 rounded-lg border-2 border-dashed border-signal/30 text-signal-strong transition-colors hover:border-signal/30 hover:bg-signal/10/50 disabled:opacity-50"
        >
          ...
          {photos.map((p) => (
            <div key={p.id} className="group relative aspect-square overflow-hidden rounded-lg bg-muted border">
              ...
              <button
                ...
                className="absolute right-1.5 top-1.5 inline-flex h-11 w-11 items-center justify-center rounded-md bg-card/90 text-destructive-strong opacity-100 shadow-sm hover:bg-card disabled:opacity-50 sm:opacity-0 sm:group-hover:opacity-100 sm:focus-visible:opacity-100"
                title="Удалить"
              >
