...
export async function logProduction(input: {
  ...
  return withReadinessTenantTransaction(input.tenantId, async (tx) => {
    ...
    */
    ...
    /*
      ПОВТОР ТОЙ ЖЕ КОМАНДЫ ПРИ ОБРЫВЕ СЕТИ — НЕ ОШИБКА, НО ОДИН P2002 ЭТОГО
      НЕ ДОКАЗЫВАЕТ. Уникальность в этой транзакции нарушает не только ключ
      команды: тот же код даёт ключ отчёта за смену (`ensureReport`) и любая
      другая уникальная пара полей. Раньше любой P2002 возвращался как «повтор
      уже принят», и проигравшая гонку запись выработки терялась молча: свая в
      отчёт не попадала, сервер отвечал 200, клиент вынимал её из очереди и
      закрывал форму (находка F-R31-1).

      Поэтому решает не код ошибки, а база: команда считается выполненной,
      только если её запись там правда есть. Чтение — отдельной короткой
      транзакцией (упавшая уже откатилась). Записи нет — ошибку не глотаем:
      клиент получит 5xx и оставит введённое в очереди.
    */
    if (typeof error === 'object' && error !== null && (error as {code?: string}).code === 'P2002') {
      const stored = await withReadinessTenantTransaction(
        input.tenantId,
        (tx) => findByCommand(tx, input.tenantId, input.entry.kind, input.clientCommandId),
      );
      if (stored) return {reportId: ''};
    }
    throw error;
  });
}
