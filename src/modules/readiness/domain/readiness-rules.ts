...
export const BLOCKER_LABELS: Record<BlockerCondition, string> = {
  ...
  /*
    Название говорит, что правило проверяет на самом деле.

    Код условия остаётся INSPECTION_BELOW_80 — он записан в JSON каждого
    опубликованного и архивного набора правил, переименование потребовало бы
    миграции данных ради косметики. Но порог 0.8 сравнивается не с долей
    заполненных пунктов, а с трёхзначным признаком осмотра (1 — смотрели
    сегодня, 0.5 — смотрели когда-то, 0 — не смотрели). Любое значение,
    кроме сегодняшнего осмотра, ниже порога, поэтому правило означает ровно
    «за сегодня машину не смотрели». Прежнее название обещало проверку
    заполненности чек-листа, которой здесь нет: владелец включал бы не то,
    что читал.
  */
  INSPECTION_BELOW_80: 'Нет осмотра за сегодня',
};
...
export const DEFAULT_READINESS_RULES: ReadinessRuleSet = {
  ...
  blockers: [
    ...
    // Осмотр за сегодня обязателен — решение владельца от 2026-09-07.
    //
    // До него правило было выключено, и допуск держался на одном критическом
    // дефекте: машина без сегодняшнего осмотра, без приёмки и с просроченным
    // нарядом объявлялась «Готова к работе». «Вернуть оператору», а не
    // «Запретить»: это не запрет, а незакрытый шаг — осмотр проводят, и
    // машина идёт в работу.
