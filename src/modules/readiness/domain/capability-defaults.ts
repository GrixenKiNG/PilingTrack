...
export const ROLE_ABILITIES: Record<ReadinessRole, readonly ReadinessAbility[]> = {
  ...
  // ПОМОЩНИК МАШИНИСТА: право есть, применить его негде.
  //
  // Без readiness.read все экраны контура для него закрыты — полоса вкладок
  // пуста, — а рабочее место машиниста отвечает отказом: смену ведёт тот, за
  // кем закреплена установка. Заводить дефект помощнику, таким образом,
  // не из чего.
  //
  // Право оставлено намеренно и до решения владельца: матрица доступов
  // редактируется и публикуется на ходу, поэтому запись здесь выражает
  // замысел, а не выдаёт доступ. Решений два — либо отдельный экран
  // помощнику, либо убрать право; молча снимать его я не стал.
