 ...
 * инженер ОТ, а не диспетчер с администратором.
 *
 * Сейчас подписывают DISPATCHER и ADMIN — это значения перечисления
 * `WorkPermitApprovalRole` в базе. Инженер ОТ наряд оформить может
 * (`readiness.permit.edit`), а согласовать — нет.
 *
 * Почему не сделано сразу: смена состава подписывающих — это миграция
 * перечисления плюс правка справочника видов работ, прав ролей и снимков
 * требований в уже выписанных нарядах. Наряды в ОРИОН не ведут (см. решение
 * о выводе их из расчёта в `readiness-rules.ts`), поэтому работа отложена до
 * момента, когда наряды понадобятся. Решение записано здесь, чтобы не
 * выяснять его заново.
 *
