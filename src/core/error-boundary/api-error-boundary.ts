...
export type DegradationFn = (ctx: ErrorContext) => NextResponse | null;
 ...
 */
export const degradeWithCache: DegradationFn = (ctx) => {
...
export function withErrorBoundary<T>(
  ...
  return handler({ request, traceId, userId, tenantId }).catch(async (error) => {
    ...
    const degradedResponse = opts.degrade(errorCtx);
    if (degradedResponse) {
