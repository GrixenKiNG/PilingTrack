...
import {rejectDefectCommand} from '@/modules/readiness/application/defects/commands';
import {rejectDefectSchema} from '@/modules/readiness/application/defects/schemas';
...
export const POST = withReadinessCommand(async (request: NextRequest, context, {params}: Params) => {
  ...
  const parsed = rejectDefectSchema.safeParse(body);
  if (!parsed.success) throw new ReadinessCommandError('VALIDATION_ERROR', 422, 'Некорректные данные отклонения', {fieldErrors: parsed.error.flatten().fieldErrors});
  const result = await withReadinessSerializableTransaction(context.tenantId,
    (tx) => rejectDefectCommand({tx, context, id,
