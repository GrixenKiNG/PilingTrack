...
const { requireAuthMock, listSitesMock } = vi.hoisted(() => ({
  requireAuthMock: vi.fn(),
  listSitesMock: vi.fn(),
}));

vi.mock('@/lib/auth', () => ({ requireAuth: requireAuthMock }));
vi.mock('@/modules/sites', () => ({ listAllSitesForAdmin: listSitesMock }));
...
describe('GET /api/sites/all', () => {
  ...
  it('returns the cached sites for a DISPATCHER', async () => {
    requireAuthMock.mockResolvedValue({ user: { id: 'd', role: 'DISPATCHER', tenantId: 'tenant-a' }, error: null });
    listSitesMock.mockResolvedValue([{ id: 's1', name: 'Site A' }]);
    const res = await GET(req());
    expect(res.status).toBe(200);
    expect((await res.json()).sites).toEqual([{ id: 's1', name: 'Site A' }]);
    expect(listSitesMock).toHaveBeenCalledWith('tenant-a', true);
  });
});
