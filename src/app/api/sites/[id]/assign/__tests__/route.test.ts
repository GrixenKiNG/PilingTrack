...
const { requireAuthMock, assignMock, unassignMock } = vi.hoisted(() => ({
...
describe('POST /api/sites/[id]/assign', () => {
  ...
  it('assigns the user to the site from the [id] param', async () => {
    requireAuthMock.mockResolvedValue(admin);
...
describe('DELETE /api/sites/[id]/assign', () => {
  ...
  it('unassigns the user from the site', async () => {
    requireAuthMock.mockResolvedValue(admin);
